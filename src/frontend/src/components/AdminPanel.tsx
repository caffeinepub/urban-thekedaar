...
import type { CalculatorLead, ContactForm, QueryForm } from "../backend";
import {
  useGetAllCalculatorLeads,
...
function LeadCard({ lead, index }: { lead: CalculatorLead; index: number }) {
  ...
  const formatAddress = (l: CalculatorLead) =>
    [
      l.address.street,
      `#${l.address.number}`,
      l.address.city,
    ...
    <div className="bg-white border border-gray-100 rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="space-y-1.5">
        ...
        <InfoRow
          icon={Calculator}
          label="Area"
          value={formatArea(lead.areaInSqFt)}
        />
        <InfoRow
          icon={Users}
          label="Floors"
          value={`${lead.numFloors} floor(s)`}
        />
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm">
                Manage all inquiries and leads
              </p>
            </div>
          </div>
          <button
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    {tab.id === "queries" &&
                      (queryFormsQuery.isLoading
                        ? "..."
                        : (queryFormsQuery.data?.length ?? 0))}
                    {tab.id === "contacts" &&
                      (contactFormsQuery.isLoading
                        ? "..."
                        : (contactFormsQuery.data?.length ?? 0))}
                    {tab.id === "leads" &&
                      (calculatorLeadsQuery.isLoading
                        ? "..."
                        : (calculatorLeadsQuery.data?.length ?? 0))}
                  </p>
